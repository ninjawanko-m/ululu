...
app.get('/api/video-status-free/:videoId', async (req, res) => {
  ...
  try {
    ...
    if (predData && predData.demoMode) {
      ...
      // 3秒後に完成 - GitHub Pagesでは5.mp4のみ使用
      // ローカルではimagesフォルダ内の全動画からランダム選択
      let localUrl = FALLBACK_VIDEO_URL; // デフォルトは5.mp4
      
      try {
        const videoFiles = fs.readdirSync(IMAGES_DIR)
          .filter(f => f.endsWith('.mp4') && !f.startsWith('sora-')) // sora-*.mp4は除外
          .map(f => `/images/${f}`);
        
        if (videoFiles.length > 0) {
          localUrl = videoFiles[Math.floor(Math.random() * videoFiles.length)];
        }
      } catch (err) {
        // GitHub Pages等、ファイルシステムにアクセスできない場合は5.mp4を使用
      }
